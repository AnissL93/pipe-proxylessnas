...
parser.add_argument(
    "--train-dir",
    default=os.path.expanduser("/ssd/dataset/imagenet/train"),
    help="path to training data",
)
parser.add_argument(
    "--val-dir",
    default=os.path.expanduser("/ssd/dataset/imagenet/val"),
    help="path to validation data",
)
parser.add_argument("--log-dir", default="./logs", help="tensorboard log directory")
parser.add_argument(
    "--format", default="./checkpoint-{epoch}.pth.tar", help="checkpoint file format"
)
parser.add_argument(
    "--fp16-allreduce",
    action="store_true",
...
parser.add_argument(
    "--val-batch-size", type=int, default=64, help="input batch size for validation"
)
...
pre_process += [transforms.ToTensor(), normalize]

train_dataset = datasets.ImageFolder(
    args.train_dir, transform=transforms.Compose(pre_process)
)
...
def adjust_learning_rate(epoch, batch_idx, type="cosine"):
    if epoch < args.warmup_epochs:
        ...
        lr_adj = 1.0 / hvd.size() * (epoch * (hvd.size() - 1) / args.warmup_epochs + 1)
    elif type == "linear":
        if epoch < 30:
            lr_adj = 1.0
        elif epoch < 60:
