    ...
    def data_provider(self):
        if self._data_provider is None:
            if self.dataset == "imagenet":
                ...
                d = dict(self.data_config)
                d.pop("resize_scale")
                d.pop("distort_color")
                d["dataset"] = "cifar10"

                self._data_provider = CifarDataProvider(**d)
                ...
                from data_providers.tiny_imagenet import TinyImagenetDataProvider

                dd = self.data_config
                dd["dataset"] = "tiny_imagenet"
                self._data_provider = TinyImagenetDataProvider(**dd)
...
class RunManager:
    def __init__(
        self, path, net, run_config: RunConfig, out_log=True, measure_latency=None
    ):
        self.path = path
        self.net = net
        ...
        self.net_on_cpu_for_latency = None  # copy.deepcopy(self.net).cpu()
        if isinstance(net, PipelineSuperProxylessNASNets):
            self.latency_estimator = None
        else:
            self.latency_estimator = LatencyEstimator(
                url=None, model_dir=None, latency_file="data/cifar10/latency1.csv"
            )
    ...
    def net_latency(self, l_type="gpu4", fast=True, given_net=None):
        ...
        if l_type == "soc":
            pl = self.net.module.current_pipeline_latency()
            bestc, bestl = self.net.module.get_best_config()
            return (pl, bestc, bestl), None
            ...
            try:
                ...
                predicted_latency += self.latency_estimator.predict(
                    ...
                    [input_size // 2, input_size // 2, net.first_conv.out_channels],
                )
                # blocks
                fsize = input_size // 2
    ...
    def print_net_info(self, measure_latency=None):
        ...
        for l_type in latency_types:
            ...
            logger.info(f"lllllllatency is {latency}")
            if self.out_log:
    ...
    def validate(self, is_test=True, net=None, use_train_mode=False, return_top5=False):
        ...
        assert data_loader is not None, "No data_loader for validation"
            ...
            for i, datas in enumerate(data_loader):
                ...
                if i % self.run_config.print_frequency == 0 or i + 1 == len(
                    ...
                    test_log = (
                        ...
                        "Top-1 acc {top1.val:.3f} ({top1.avg:.3f})".format(
                            i,
                            len(data_loader) - 1,
    ...
    def train(self, print_top5=False):
        ...
        for epoch in range(self.start_epoch, self.run_config.n_epochs):
            ...
            train_top1, train_top5 = self.train_one_epoch(
                ...
                lambda i, batch_time, data_time, losses, top1, top5, new_lr: train_log_func(
                    epoch, i, batch_time, data_time, losses, top1, top5, new_lr
                ),
            )
            ...
            if (epoch + 1) % self.run_config.validation_frequency == 0:
                ...
                val_log = (
                    "Valid [{0}/{1}]\tloss {2:.3f}\ttop-1 acc {3:.3f} ({4:.3f})".format(
                        epoch + 1,
                ...
                if print_top5:
                    val_log += "\ttop-5 acc {0:.3f}\tTrain top-1 {top1.avg:.3f}\ttop-5 {top5.avg:.3f}".format(
                        val_acc5, top1=train_top1, top5=train_top5
                    )
                else:
