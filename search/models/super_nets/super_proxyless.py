...
class SuperProxylessNASNets(ProxylessNASNets):
    def __init__(
        ...
        # feature mix layer
        last_channel = (
            make_divisible(self._last_channel * width_mult, 8)
            if width_mult > 1.0
    ...
    def weight_parameters(self):
        for name, param in self.named_parameters():
            if (
                "AP_path_alpha" not in name
                and "AP_path_wb" not in name
                and "PROC" not in name
            ):
                yield param

    def proc_weight_parameters(self):
        for name, param in self.named_parameters():
            if "PROC_param" in name:
                yield param
    ...
    def init_proc_params(self):
        for param in self.proc_weight_parameters():
    ...
    def expected_latency(self, latency_model: LatencyEstimator):
        ...
        print("========== compute expected latency")
        expected_latency = 0
        # first conv
        expected_latency += latency_model.predict(
            "Conv",
            [input_size, input_size, 3],
            [input_size // 2, input_size // 2, self.first_conv.out_channels],
        )
        # blocks
        fsize = input_size // 2
        i = 0
    ...
    def convert_to_normal_net(self):
        ...
        return ProxylessNASNets(
            self.first_conv, list(self.blocks), self.feature_mix_layer, self.classifier
        )


# class BlockInfo(object):
#     def __init__(self, layer : MyModule, in_hw):
#         self._layer = layer
#         self._in_hw = in_hw
#         pass
...
#     @property
#     def output_hw(self):
#         return self._layer.get_outhw(self._in_hw)
