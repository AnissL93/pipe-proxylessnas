...
def build_candidate_ops_for_cifar(candidate_ops, inc, outc, stride):
    if candidate_ops is None:
        raise ValueError("please specify a candidate set")

    name2op = {
        "Conv_3x3": lambda in_c, out_c, S: ConvLayer(in_c, out_c, 3, S),
        "Conv_5x5": lambda in_c, out_c, S: ConvLayer(in_c, out_c, 5, S),
        "Pool_5x5": lambda in_c, out_c, S: PoolingLayer(in_c, out_c, "max", 5, S),
        "Id": lambda in_C, out_C, S: IdentityLayer(in_C, out_C, ops_order=None),
    }

    return [name2op[name](inc, outc, stride) for name in candidate_ops]


def build_candidate_ops(
    candidate_ops, in_channels, out_channels, stride, ops_order, dropout_rate=0.0
):
    if candidate_ops is None:
        raise ValueError("please specify a candidate set")
    ...
    name2ops.update(
        {
            "3x3_MBConv1": lambda in_C, out_C, S: MBInvertedConvLayer(
                in_C, out_C, 3, S, 1, None, dropout_rate
            ),
            "3x3_MBConv2": lambda in_C, out_C, S: MBInvertedConvLayer(
                in_C, out_C, 3, S, 2, None, dropout_rate
            ),
            "3x3_MBConv3": lambda in_C, out_C, S: MBInvertedConvLayer(
                in_C, out_C, 3, S, 3, None, dropout_rate
            ),
            "3x3_MBConv4": lambda in_C, out_C, S: MBInvertedConvLayer(
                in_C, out_C, 3, S, 4, None, dropout_rate
            ),
            "3x3_MBConv5": lambda in_C, out_C, S: MBInvertedConvLayer(
                in_C, out_C, 3, S, 5, None, dropout_rate
            ),
            "3x3_MBConv6": lambda in_C, out_C, S: MBInvertedConvLayer(
                in_C, out_C, 3, S, 6, None, dropout_rate
            ),
            #######################################################################################
            "5x5_MBConv1": lambda in_C, out_C, S: MBInvertedConvLayer(
                in_C, out_C, 5, S, 1, None, dropout_rate
            ),
            "5x5_MBConv2": lambda in_C, out_C, S: MBInvertedConvLayer(
                in_C, out_C, 5, S, 2, None, dropout_rate
            ),
            "5x5_MBConv3": lambda in_C, out_C, S: MBInvertedConvLayer(
                in_C, out_C, 5, S, 3, None, dropout_rate
            ),
            "5x5_MBConv4": lambda in_C, out_C, S: MBInvertedConvLayer(
                in_C, out_C, 5, S, 4, None, dropout_rate
            ),
            "5x5_MBConv5": lambda in_C, out_C, S: MBInvertedConvLayer(
                in_C, out_C, 5, S, 5, None, dropout_rate
            ),
            "5x5_MBConv6": lambda in_C, out_C, S: MBInvertedConvLayer(
                in_C, out_C, 5, S, 6, None, dropout_rate
            ),
            #######################################################################################
            "7x7_MBConv1": lambda in_C, out_C, S: MBInvertedConvLayer(
                in_C, out_C, 7, S, 1, None, dropout_rate
            ),
            "7x7_MBConv2": lambda in_C, out_C, S: MBInvertedConvLayer(
                in_C, out_C, 7, S, 2, None, dropout_rate
            ),
            "7x7_MBConv3": lambda in_C, out_C, S: MBInvertedConvLayer(
                in_C, out_C, 7, S, 3, None, dropout_rate
            ),
            "7x7_MBConv4": lambda in_C, out_C, S: MBInvertedConvLayer(
                in_C, out_C, 7, S, 4, None, dropout_rate
            ),
            "7x7_MBConv5": lambda in_C, out_C, S: MBInvertedConvLayer(
                in_C, out_C, 7, S, 5, None, dropout_rate
            ),
            "7x7_MBConv6": lambda in_C, out_C, S: MBInvertedConvLayer(
                in_C, out_C, 7, S, 6, None, dropout_rate
            ),
        }
    )
...
class MixedEdge(MyModule):
    ...
    def forward(self, x):
        if MixedEdge.MODE == "full" or MixedEdge.MODE == "two":
            ...
            # simply run the active candidates op
                ...
                def backward(_x, _output, grad_output):
                    ...
                    with torch.no_grad():
                        for k in range(len(candidate_ops)):
                            if k != active_id:
                                # for not actiavted candidates, compute it
                                out_k = candidate_ops[k](_x.data)
                            else:
                                # for active candidate, just get the output, probably because this is computed in the forward process.
                                out_k = _output.data
            ...
            output = ArchGradientFunction.apply(
                ...
                backward_function(
                    self.candidate_ops, self.active_index[0], self.AP_path_wb
                ),
            )
        else:
    ...
    def module_str(self):
        ...
        return "Mix(%d - %s, %.3f)" % (
            chosen_index,
            self.candidate_ops[chosen_index].module_str,
            probs,
        )
    ...
    def binarize(self):
        ...
        if MixedEdge.MODE == "two":
            ...
            probs_slice = F.softmax(
                torch.stack([self.AP_path_alpha[idx] for idx in sample_op]), dim=0
            )
    ...
    def set_arch_param_grad(self):
        ...
        if MixedEdge.MODE == "two":
            ...
            probs_slice = F.softmax(
                torch.stack([self.AP_path_alpha[idx] for idx in involved_idx]), dim=0
            ).data
            for i in range(2):
                for j in range(2):
...
if __name__ == "__main__":
    ...
    MixedEdge.MODE = "two"
    edge = MixedEdge(
        candidate_ops=build_candidate_ops(conv_candidates, 64, 128, 1, "weight_bn_act")
    )
    edge.binarize()
    print(edge.active_index)
    print(edge.inactive_index)
    print(edge.AP_path_alpha)
    print(edge.chosen_index)

    print(edge)
    print(edge.module_str)

    # cifar_cands = ["Conv_3x3", "Conv_5x5", "Pool_5x5", "Id"]
    # MixedEdge.MODE = "two"
    # cifar_edge = MixedEdge(candidate_ops=build_candidate_ops(cifar_cands, 64, 128, 1, "weight_bn_act"))
