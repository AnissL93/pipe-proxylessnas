...
parser.add_argument("--split_points", type=str, default="10,15")
""" RL hyper-parameters """
...
if __name__ == '__main__':
    ...
    if args.grad_reg_loss_type == "pipeline":
        ...
        if len(points) == 1:
            config= cpu_config(points)
        else:
            config = cpu_gpu_config(points)
    ...
    if args.resume:
        try:
            ...
            home = str(Path.home())
            print('Workspace/Exp/arch_search/%s_ProxylessNAS_%.2f_%s/warmup.pth.tar' % (run_config.dataset, args.width_mult, width_stages_str))
