...
class ProcSearchConfig:
    def __init__(self, init_type, lr, decay):
        self.init_type = init_type
        self.lr = lr
        self.decay = decay

    def build_optimizer(self, params):
        return torch.optim.Adam(params, self.lr, weight_decay=self.decay)
...
class GradientArchSearchConfig(ArchSearchConfig):
    ...
    def add_regularization_loss(self, ce_loss, expected_value):
        ...
        print(f"{self.reg_loss_type=}, {self.ref_value=}")
        ...
        elif self.reg_loss_type == "pipeline":
            assert isinstance(expected_value, tuple)
            print(expected_value)
            val = expected_value[0]
            diff_loss = expected_value[1]
            alpha = self.reg_loss_params.get("alpha", 1)
            beta = self.reg_loss_params.get("beta", 0.6)
            reg_loss = (torch.log(val) / math.log(self.ref_value)) ** beta
            loss = alpha * ce_loss * val
            logger.info(f"CE loss: {ce_loss}, latency: {expected_value}, loss = {loss}")
            return loss
...
class RLArchSearchConfig(ArchSearchConfig):
    ...
    def calculate_reward(self, net_info):
        ...
        if self.target_hardware is None:
            ...
            return acc * (
                (self.ref_value / net_info[self.target_hardware]) ** self.tradeoff_ratio
            )
...
class ArchSearchRunManager:
    def __init__(
        ...
        proc_search_config: ProcSearchConfig,
        weight_only=False,
    ):
        ...
        self.proc_search_config = proc_search_config
        ...
        # add proc optimizer
        if self.proc_search_config is not None:
            self.proc_optimizer = self.proc_search_config.build_optimizer(
                self.net.proc_weight_parameters()
            )
            self.net.init_proc_params()
        else:
            self.proc_optimizer = None
    ...
    def write_log(self, log_str, prefix, should_print=True, end="\n"):
        with open(
            os.path.join(self.run_manager.logs_path, "%s.log" % prefix), "a"
        ) as fout:
    ...
    def validate(self):
        ...
        valid_res = self.run_manager.validate(
            is_test=True, use_train_mode=True, return_top5=True
        )
    ...
    def warm_up(self, warmup_epochs=25):
        ...
        print(f"{self.warmup_epoch=}, {warmup_epochs=}")
        if isinstance(self.warmup_epoch, tuple):
            we = self.warmup_epoch[0]
        else:
            we = self.warmup_epoch
        for epoch in range(we, warmup_epochs):
            ...
            for i, data in enumerate(data_loader):
                ...
                if (
                    i % self.run_manager.run_config.print_frequency == 0
                    or i + 1 == nBatch
                ):
                    batch_log = (
                        "Warmup Train [{0}][{1}/{2}]\t"
                        ...
                        "Top-5 acc {top5.val:.3f} ({top5.avg:.3f})\tlr {lr:.5f}".format(
                            epoch + 1,
                            i,
                            nBatch - 1,
            ...
            val_log = (
                "Warmup Valid [{0}/{1}]\tloss {2:.3f}\ttop-1 acc {3:.3f}\ttop-5 acc {4:.3f}\t"
                "Train top-1 {top1.avg:.3f}\ttop-5 {top5.avg:.3f}\tflops: {5:.1f}M".format(
                    epoch + 1,
                    warmup_epochs,
                    *valid_res,
                    flops / 1e6,
            ...
            if self.arch_search_config.target_hardware not in [None, "flops"]:
                val_log += (
                    "\t"
                    + self.arch_search_config.target_hardware
                    + ": %s" % str(latency)
                )
    ...
    def train_with_pipeline(self, fix_weight=False, weight_only=False):
        """Train with pipeline processor assignment weights

        Args:
            stage (int, optional): the stage of current training. Defaults to 0.
            stage0: train network weights
            stage1: train arch param
            stage2: train processor weights
        """
        data_loader = self.run_manager.run_config.train_loader
        nBatch = len(data_loader)
        if fix_weight:
        ...
        proc_weight_num = len(list(self.net.proc_weight_parameters()))
        print(
            "#arch_params: %d\t#binary_gates: %d\t#weight_params: %d\t#proc_weight_parmas: %d"
            % (arch_param_num, binary_gates_num, weight_param_num, proc_weight_num)
        )

        arch_update_schedule = self.arch_search_config.get_update_schedule(nBatch)
        if self.proc_search_config is None:
            proc_update_schedule = None
        else:
            proc_update_schedule = self.proc_search_config.get_update_schedule(nBatch)

        ## ir scheduler
        # scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(
        #     self.run_manager.optimizer, T_max=200
        # )

        for epoch in range(
            self.run_manager.start_epoch, self.run_manager.run_config.n_epochs
        ):
            ...
            for i, datas in enumerate(data_loader):
                ...
                if not weight_only and epoch > 0:
                    # update architecture parameters according to update_schedule
                    for j in range(arch_update_schedule.get(i, 0)):
                        ...
                        if isinstance(self.arch_search_config, RLArchSearchConfig):
                            ...
                            log_str = (
                                "REINFORCE [%d-%d]\tTime %.4f\tMean Reward %.4f\t%s"
                                % (
                                    epoch + 1,
                                    i,
                                    used_time,
                            ...
                            used_time = time.time() - start_time
                            log_str = (
                                "Architecture [%d-%d]\t Time %.4f\t Loss %.4f\t %s %s"
                                % (
                                    epoch + 1,
                                    i,
                                    used_time,
                            ...
                            self.write_log(
                                log_str, prefix="gradient", should_print=False
                            )
                        else:
                            raise ValueError(
                ...
                if (
                    ...
                    and epoch > 0
                    and epoch < 2000
                    and proc_update_schedule is not None
                ):
                    # logger.info("============ do proc gradient step  ============ ")
                    # logger.info(f"weight only is {weight_only}")
                    for j in range(proc_update_schedule.get(i, 0)):
                        start_time = time.time()
                        (
                            exp_value,
                            proc_assign,
                            ...
                            bestc,
                            bestl,
                        ) = self.proc_gradient_step()
                        used_time = time.time() - start_time
                        log_str = (
                            "Processor [%d-%d]\t Time %.4f\t Loss %.4f\t %s %s %s %s"
                            % (
                                epoch + 1,
                                i,
                                used_time,
                ...
                if (
                    ...
                    lr = self.run_manager.optimizer.param_groups[0]["lr"]
                    batch_log = (
                        "Train [{0}][{1}/{2}]\t"
                        "Time {batch_time.val:.3f} ({batch_time.avg:.3f})\t"
            ...
            self.write_log(
                "-" * 30 + "Current Architecture [%d]" % (epoch + 1) + "-" * 30,
                prefix="arch",
            )
            ...
            if (epoch + 1) % self.run_manager.run_config.validation_frequency == 0:
                ...
                val_log = (
                    "Valid [{0}/{1}]\tloss {2:.3f}\ttop-1 acc {3:.3f} ({4:.3f})\ttop-5 acc {5:.3f}\t"
                    "Train top-1 {top1.avg:.3f}\ttop-5 {top5.avg:.3f}\t"
                    "Entropy {entropy.val:.5f}\t"
                    "Latency-{6}: {7} \tFlops: {8:.2f}M".format(
                        epoch + 1,
                        self.run_manager.run_config.n_epochs,
                        val_loss,
                        val_top1,
    ...
    def train(self, fix_net_weights=False):
        writer = SummaryWriter(log_dir=self.run_manager.logs_path)
        data_loader = self.run_manager.run_config.train_loader
        nBatch = len(data_loader)
        if fix_net_weights:
            data_loader = [(0, 0)] * nBatch
        ...
        for epoch in range(
            ...
            for i, datas in enumerate(data_loader):
                ...
                if (
                    ...
                    writer.add_scalar("Top1/train", top1, epoch + 1)
                    writer.add_scalar("Top5/train", top5, epoch + 1)
                    writer.add_scalar("lr/train", lr, epoch + 1)
            ...
            if (epoch + 1) % self.run_manager.run_config.validation_frequency == 0:
                ...
                val_log = (
                    ...
                    "Train top-1 {top1.avg:.3f}\ttop-5 {top5.avg:.3f}\t"
                    "Entropy {entropy.val:.5f}\t"
                    "Latency-{6}: {7:.3f}ms\tFlops: {8:.2f}M".format(
                        epoch + 1,
                        self.run_manager.run_config.n_epochs,
                        val_loss,
                        val_top1,
                ...
                self.run_manager.write_log(val_log, "valid")
                writer.add_scalar("Loss/test", losses, epoch + 1)
                writer.add_scalar("Top1/test", top1, epoch + 1)
                writer.add_scalar("Top5/test", top5, epoch + 1)
                writer.add_scalar("lr/test", lr, epoch + 1)
                writer.add_scalar("latency/test", latency, epoch + 1)
                logger.info("write validation finish")
    ...
    def rl_update_step(self, fast=True):
        ...
        for i in range(self.arch_search_config.batch_size):
            ...
            if self.arch_search_config.target_hardware is None:
                ...
                (
                    net_info[self.arch_search_config.target_hardware],
                    _,
    ...
    def gradient_step(self):
        ...
        # sample a batch of data from validation set
        data = self.run_manager.run_config.valid_next_batch
        if isinstance(data, dict):
            images = data["image"]
            labels = data["label"]
        else:
        ...
        if self.arch_search_config.target_hardware is None:
            ...
            expected_value = self.net.expected_latency(
                self.run_manager.latency_estimator
            )
        elif self.arch_search_config.target_hardware == "flops":
            data_shape = [1] + list(
        ...
        elif self.arch_search_config.target_hardware == "soc":
            # expected_value = self.net.expected_latency()
            expected_value = self.net.latency_loss()
            # expected_value = 0.
        else:
            raise NotImplementedError
        ...
        self.write_log(
            "(%.4f, %.4f, %.4f)" % (time2 - time1, time3 - time2, time4 - time3),
            "gradient",
            should_print=False,
            end="\t",
        )
    ...
    def proc_gradient_step(self):
        ...
        # loss = self.arch_search_config.add_regularization_loss(ce_loss, expected_value)
        val, diff = self.net.latency_loss()
        latency_loss = val + diff

        self.run_manager.net.zero_grad()  # zero grads of weight_param, arch_param & binary_param
        latency_loss.backward()

        self.proc_optimizer.step()

        expected_lat = self.net.current_pipeline_latency(False)
        bestc, bestl = self.net.get_best_config()
        assign = self.net.current_assignment

        return expected_lat, assign, latency_loss.data.item(), str(bestc), bestl

    def proc_grad_step(self):
        assert isinstance(self.arch_search_config, GradientArchSearchConfig)
        assert self.arch_search_config.target_hardware == "soc"
        ...
        bestc, bestl = self.net.get_best_config()
        assign = self.net.current_assignment
        latency_loss = self.net.latency_loss()
