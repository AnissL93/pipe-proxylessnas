...
from models import ImagenetRunConfig, Cifar10RunConfig, TinyImagenetRunConfig
...
parser.add_argument('--eval_pipe_only', default=1, type=int)
...
parser.add_argument('--dataset', type=str, default='tiny_imagenet', choices=['imagenet', 'cifar10', 'cifar100', "tiny_imagenet"])
...
parser.add_argument('--n_cell_stages', type=str, default='3,3,3,3,1')
parser.add_argument('--stride_stages', type=str, default='2,2,1,2,1')
parser.add_argument('--width_mult', type=float, default=1.0)
parser.add_argument('--last_channel', type=int, default=512)
...
parser.add_argument('--target_hardware', type=str, default=None, choices=['mobile', 'cpu', 'gpu8', 'flops', "soc", None])
...
parser.add_argument('--grad_reg_loss_type', type=str, default='mul#log', choices=['add#linear', 'mul#log', "pipeline"])
...
parser.add_argument("--latency_data", type=str, default="data/cifar10/latency1.csv")
""" RL hyper-parameters """
...
parser.add_argument('--complete_sep', type=int, default=0)
parser.add_argument('--fix_arch', type=int, default=0)

def write_args(args):
    out_path = args.path

    with open(out_path + "/args.log", "w") as fp:
        for arg_name, arg_value in vars(args).items():
            fp.write(f"{arg_name} = {arg_value}\n")
...
if __name__ == '__main__':
    ...
    if args.dataset == "imagenet":
        ...
        run_config = TinyImagenetRunConfig(
            **args.__dict__
        )
    else:
    ...
    if args.grad_reg_loss_type == "pipeline":
        logger.info(f"{run_config.data_provider.n_classes=}")
        super_net = PipelineSuperProxylessNASNets(
            ...
            last_c=args.last_channel,
            lat_data=args.latency_data,
            input_size=run_config.data_provider.image_size
        )
        print(super_net)
    else:
        super_net = SuperProxylessNASNets(
            width_stages=args.width_stages, n_cell_stages=args.n_cell_stages, stride_stages=args.stride_stages,
            conv_candidates=args.conv_candidates, n_classes=run_config.data_provider.n_classes, width_mult=args.width_mult,
            bn_param=(args.bn_momentum, args.bn_eps), dropout_rate=args.dropout, last_channel=args.last_channel,
            input_size=run_config.data_provider.image_size
        )
    ...
    if args.grad_reg_loss_type == "pipeline":
        proc_search_config = ProcSearchConfig("normal", lr=1e-3, decay=0)
    else:
        proc_search_config = None
    ...
    arch_search_run_manager = ArchSearchRunManager(args.path, super_net, run_config, arch_search_config, proc_search_config)
    # resume
    if args.resume:
        try:
    ...
    if args.eval_pipe_only:
        # copy to pipeline super proxyless nas not
        pipe_net = PipelineSuperProxylessNASNets(
            width_stages=args.width_stages, n_cell_stages=args.n_cell_stages, stride_stages=args.stride_stages,
            conv_candidates=args.conv_candidates, n_classes=run_config.data_provider.n_classes, width_mult=args.width_mult,
            bn_param=(args.bn_momentum, args.bn_eps), dropout_rate=args.dropout,
            last_c=args.last_channel,
        ...
        for key, value in vars(arch_search_run_manager.net).items():
            setattr(pipe_net, key, value)

        print(pipe_net.get_best_config())
        exit(-1)
    # warmup
    ...
    arch_search_run_manager.train_with_pipeline(args.debug)
